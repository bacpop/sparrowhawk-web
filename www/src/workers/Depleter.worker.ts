...
interface FilterMessage    { filter: true; file: File; revReads: File | null; deplete: boolean; abs_threshold: number; rel_threshold: number; sampleName: string; }
...
ctx.onmessage = (evt: MessageEvent<WorkerMessage>) => {
    ...
    if ('loadIndex' in evt.data && evt.data.loadIndex) {
        ...
        depleter.filterReads(m.file, m.revReads, m.deplete, m.abs_threshold, m.rel_threshold, m.sampleName);
