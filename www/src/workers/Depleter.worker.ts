...
interface FilterMessage    { filter: true; file: File; deplete: boolean; abs_threshold: number; rel_threshold: number; }
...
ctx.onmessage = (evt: MessageEvent<WorkerMessage>) => {
    ...
    if ('loadIndex' in evt.data && evt.data.loadIndex) {
        ...
        depleter.filterReads(m.file, m.deplete, m.abs_threshold, m.rel_threshold);
